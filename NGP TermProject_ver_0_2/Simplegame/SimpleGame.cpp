...
void MouseInput(int button, int state, int x, int y)
{
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_UP) {
		if (g_LButtonDown) {
			...
			{
				g_SceneMgr->CreateObject(x - (WINDOWHALFSIZE_WIDTH), (WINDOWHALFSIZE_HEIGHT)-y, OBJECT_CHARACTER, TEAM_1);
				g_mouseCooltime = 0.f;
			}
		}
		g_LButtonDown = false;
...
void KeyInput(unsigned char key, int x, int y)
{
	switch (key)
	{
		case GLUT_KEY_UP:
		{
			//g_SceneMgr->GetActorObject()
			for (int i = 0; i < g_SceneMgr->GetObjectCnt(); ++i)
			{
				if (g_SceneMgr->m_objects[i]->GetType() == OBJECT_CHARACTER)
					g_SceneMgr->m_objects[i]->SetMovePositionX(5.0f);
			}
...
int main(int argc, char **argv)
{
   ...
   for (int i = 0; i < 50; ++i)
   {
	   g_SceneMgr->CreateObject(0, 0, OBJECT_ARROW, TEAM_1);
   }
